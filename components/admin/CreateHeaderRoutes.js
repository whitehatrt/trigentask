...
const CreateHeaderRoutes = () => {
  const [headerData, setheaderData] = useState({ path: "", label: "" });
  const handleSubmit = async (e) => {
    e.preventDefault();

    const res = await axios.post("headerRoutes", headerData);
  ...
  return (
    <>
      <button
        ...
        data-bs-target="#createheaderroutes"
      >
        Create Header Routes
      </button>
      <div
        className="modal fade"
        id="createheaderroutes"
        tabIndex={-1}
        ...
        <div className="modal-dialog modal-dialog-centered modal-dialog-scrollable">
          <div className="modal-content rounded-5 shadow">
            <div className="modal-header p-5 pb-4 border-bottom-0">
              <h5 className="modal-title">Create Header Routes</h5>
...
export default CreateHeaderRoutes;
